...
- Monorepo: Turborepo
- Frontend: Next.js 16, React 19, Tailwind CSS 4, HeroUI React v3
...
- `packages/design-system` owns the HeroUI theme, fonts, and shared visual tokens.
- Application code imports interactive primitives directly from `@heroui/react` and uses the checked-in HeroUI documentation described below.
...
- Use HeroUI v3 primitives and compound APIs before writing custom visual markup.
- Never add shadcn, Base UI, Radix UI, or a pass-through wrapper around a HeroUI primitive.
- Use `onPress` for HeroUI interactions and preserve native Next.js links for routing.
- Keep one primary action per page.
- Preserve the calm Rantau shell, readable density, visible focus, and responsive sidebar.
...
For HeroUI component, styling, or migration work, search `.heroui-docs/react` and read the relevant component guide or demo before choosing an API. The checked-in documentation matches this project; use its current v3 examples.
